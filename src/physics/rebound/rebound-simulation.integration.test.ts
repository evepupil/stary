...
async function loadSimulation(bodies: readonly BodyState[]): Promise<ReboundSimulation> {
  return createReboundSimulation(bodies, { locateFile: () => reboundWasmPath });
}
...
async function runWithTimeScale(timeScale: number, realMilliseconds: number) {
  ...
  const runtime = new PhysicsWorkerRuntime({
    createSimulation: (bodies) => loadSimulation(bodies),
    postMessage: (message) => {
