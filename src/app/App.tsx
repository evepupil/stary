...
function ProbeRow({ label, state }: ProbeRowProps) {
  return (
    <div>
      <dt>{label}</dt>
      <dd aria-live="polite" data-status={state.status}>
  ...
  return (
    <main className="foundation-shell">
      <section aria-labelledby="foundation-title" className="foundation-status">
        ...
        <dl>
          <ProbeRow label="渲染模块路径 / 运行时探针" state={probes.renderer} />
          <ProbeRow label="Worker 模块路径 / 运行时探针" state={probes.worker} />
          <ProbeRow label="WASM 资源路径 / 运行时探针" state={probes.wasm} />
        </dl>
      </section>
    </main>
  );
