...
以下出版信息已在 2026-07-15 通过 Crossref 元数据核对。Task 1 会把论文公式编号、参数和黄金样例写入版本化引用表，运行时不会访问外部网站。
...
质量比和擦碰参与质量继续使用论文的 reduced-mass scaling 与 `alpha` 修正。Task 1 必须把论文对应公式转成固定黄金样例，禁止凭经验补系数。
...
完成门槛：纯函数没有未来源的物理常数；质量、线动量、角动量和能量账本达到硬门槛。
